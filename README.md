...
I therefore expose raw observations, quality flags and transport states separately. A finite but flagged value counts toward availability, not toward validity. The illustrative high-NH₃ threshold is a demonstration setting, not a regulatory or occupational limit.
...
The firmware does not calculate NH₃ from MQ2 voltage. Its NH₃ fields remain empty/null. MQTT is unconfigured by default. The firmware RAM queue holds 120 records; overflows are reported, and available SD records remain accessible. Its QoS 0 publishing does not prove delivery. See the explicit [implementation limits](docs/limitations.md).

## Digital twin and fault injection

The twin represents measurement-system operational state, not a complete livestock building. Use **Fault Injection** to apply NORMAL, HIGH_NH3, SENSOR_DISCONNECTED, SENSOR_TIMEOUT, SENSOR_DRIFT, INVALID_READING, NETWORK_OFFLINE, STORAGE_FAILURE or RECOVERY. Each action acquires a sample immediately.

During network failure, local acquisition continues and pending telemetry grows. RECOVERY services the pending queue on the next sample and records synchronization. The Python receiver acknowledges in process; this is a controlled no-gap demonstration within a running session. During combined network and storage failures, both pending queues remain visible. They are volatile and do not survive process termination.
...
Plots show the time series, raw/calibrated scatter with a 1:1 line, residuals, and errors versus time, temperature and humidity. This transparent model does not eliminate environmental cross-sensitivity or drift. The simulated reference also has error and is not ground truth.
