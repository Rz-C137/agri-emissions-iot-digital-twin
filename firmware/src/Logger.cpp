...
bool Logger::append(const Measurement& m) {
    ...
    if (!file.size()) file.println("timestamp,timestamp_status,node_id,sequence,uptime_ms,temperature_c,relative_humidity_pct,gas_raw,nh3_raw_ppm,nh3_calibrated_ppm,nh3_reference_ppm,environmental_sensor_status,gas_channel_status,sensor_status,network_status,storage_status,quality_code,quality_flags,buffered,scenario,simulated");
    String row = csv(m);
